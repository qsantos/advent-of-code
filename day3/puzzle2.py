...
def intersections(path1: Path, path2: Path) -> Iterable[int]:
    visited: Dict[Tuple[int, int], int] = {}
    for step, pos in enumerate(walk(path1)):
        if pos not in visited:
            visited[pos] = step + 1
    for step, pos in enumerate(walk(path2)):
        if pos in visited:
            yield step + 1 + visited[pos]


def closest_intersection(path1: Path, path2: Path) -> int:
    return min(intersections(path1, path2))


def main() -> None:
    assert closest_intersection(parse_path('R8,U5,L5,D3'), parse_path('U7,R6,D4,L4')) == 30
